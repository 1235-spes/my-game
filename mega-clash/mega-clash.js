...
// الرموز
const SYMBOLS = [
  { img: "عجلة.صفرا.لعبة.رابعة.jpg", payouts: { 3: 1, 4: 2, 5: 4 } },
  { img: "عجلة.زرقا.لعبة.رابعة.jpg", payouts: { 3: 2, 4: 4, 5: 8 } },
  { img: "عجلة.خضرا.لعبة.رابعة.jpg", payouts: { 3: 3, 4: 6, 5: 12 } },
  { img: "رقم.عشر.لعبة.رابعة.jpg", payouts: { 3: 4, 4: 8, 5: 16 } },
  { img: "رقم.تسعة.لعبة.رابعة.jpg", payouts: { 3: 5, 4: 10, 5: 20 } },
  { img: "تاج.لعبة.رابعة.jpg", payouts: { 3: 6, 4: 12, 5: 25 } },
  { img: "ويلد.لعبة.رابعة.jpg", payouts: { 3: 2, 4: 5, 5: 10 } },
  { img: "تاج.عادي.لعبة.رابعة.jpg", payouts: { 3: 3, 4: 7, 5: 14 } },
  { img: "بستون.لعبة.رابعة.jpg", payouts: { 3: 4, 4: 9, 5: 18 } },
  { img: "اجر.حصان.لعبة.رابعة.jpg", payouts: { 3: 5, 4: 11, 5: 22 } },
  { img: "كنز.لعبة.رابعة.jpg", payouts: { 3: 7, 4: 15, 5: 30 } }
];
...
function generateFinalResult() {
  ...
  for (let col = 0; col < COLS; col++) {
    ...
    for (let row = 0; row < ROWS; row++) {
      ...
      if (
        symbol.img === "ويلد.لعبة.رابعة.jpg" &&
        (col === 0 || col === COLS - 1)
      ) {
        // نعيد الاختيار مرة ثانية بدون Wild
        do {
          symbol = getRandomSymbol();
        } while (symbol.img === "ويلد.لعبة.رابعة.jpg");
      }
...
function highlightWild() {
  ...
  reels.forEach(reel => {
    ...
    if (hasWild) {
      ...
      imgs.forEach(img => {
        img.src = "../images/ويلد.لعبة.رابعة.jpg";
...
let hasWild = symbols.some(s => s.img === "ويلد.لعبة.رابعة.jpg");
...
if (hasWild) {
  isWildColumn = true;
    ...
    for (let i = 1; i < symbols.length; i++) {
      ...
      if (symbols[i].img === "ويلد.لعبة.رابعة.jpg") {
        match++;
        continue;
      }
...
if (match >= 3) {
  ...
  if (symbols.some(s => s.img === "ويلد.لعبة.رابعة.jpg")) {
    winningImg = "ويلد.لعبة.رابعة.jpg"; // يعتبر Wild هو الفائز
  } else {
    winningImg = base.img;
  }
  ...
  if (winningImg) {
    ...
    highlightWild();
      if (winningImg === "ويلد.لعبة.رابعة.jpg") {
    wildSound.currentTime = 0;
    wildSound.play(); 
   }
    }
