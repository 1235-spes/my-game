...
// الرموز
const SYMBOLS = [
  { img: "لعبة.خامسي.توت.بري.jpg", payouts: { 3: 1, 4: 2, 5: 4 } },
  { img: "لعبة.خامسة برتقال.jpg", payouts: { 3: 2, 4: 4, 5: 8 } },
  { img: "لعبة.خامسي.خوخ.jpg", payouts: { 3: 3, 4: 6, 5: 12 } },
  { img: "لعبة.خامسي.جبس.jpg", payouts: { 3: 4, 4: 8, 5: 16 } },
  { img: "لعبة.خامسي.نجاص.اخضر.jpg", payouts: { 3: 5, 4: 10, 5: 20 } },
  { img: "لعبة.خامسي.منغا.jpg", payouts: { 3: 6, 4: 12, 5: 25 } },
  { img: "لعبة.خامسي.توت.اسود.jpg", payouts: { 3: 2, 4: 5, 5: 10 } },
  { img: "لعبة.خامسي.نجمي.jpg", payouts: { 3: 3, 4: 7, 5: 14 } },
  { img: "لعبة.خامسي.جرس.jpg", payouts: { 3: 4, 4: 9, 5: 18 } },
  { img: "لعبة.خامسي.سبعة.jpg", payouts: { 3: 5, 4: 11, 5: 22 } },
  { img: "لعبة.خاسي.تفاح.احمر.jpg", payouts: { 3: 7, 4: 15, 5: 30 } }
];
...
function generateFinalResult() {
  ...
  for (let col = 0; col < COLS; col++) {
    ...
    for (let row = 0; row < ROWS; row++) {
      ...
      if (
        symbol.img === "لعبة.خامسي.سبعة.jpg" &&
        (col === 0 || col === COLS - 1)
      ) {
        // نعيد الاختيار مرة ثانية بدون Wild
        do {
          symbol = getRandomSymbol();
        } while (symbol.img === "لعبة.خامسي.سبعة.jpg");
      }
...
let hasWild = symbols.some(s => s.img === "لعبة.خامسي.سبعة.jpg");
...
if (hasWild) {
  isWildColumn = true;
    ...
    for (let i = 1; i < symbols.length; i++) {
      ...
      if (symbols[i].img === "لعبة.خامسي.سبعة.jpg") {
        match++;
        continue;
      }
...
if (match >= 3) {
  ...
  if (symbols.some(s => s.img === "لعبة.خامسي.سبعة.jpg")) {
    winningImg = "لعبة.خامسي.سبعة.jpg"; // يعتبر Wild هو الفائز
  } else {
    winningImg = base.img;
  }
  ...
  if (winningImg) {
    ...
    highlightWild();
      if (winningImg === "لعبة.خامسي.سبعة.jpg") {
    wildSound.currentTime = 0;
    wildSound.play(); 
   }
    }
