...
// الرموز
const SYMBOLS = [
  { img: "لعبة.خامسي.افوكادو.jpg", payouts: { 3: 1, 4: 2, 5: 4 } },
  { img: "لعبة.خاسي.تفاح.احمر.jpg", payouts: { 3: 2, 4: 4, 5: 8 } },
  { img: "لعبة.خامسي.كرز.jpg", payouts: { 3: 3, 4: 6, 5: 12 } },
  { img: "لعبة.خامسي.تفاح.اخضر.jpg", payouts: { 3: 4, 4: 8, 5: 16 } },
  { img: "لعبة.خامسي.موز.jpg", payouts: { 3: 5, 4: 10, 5: 20 } },
  { img: "لعبة.خامسي.فرز.jpg", payouts: { 3: 6, 4: 12, 5: 25 } },
  { img: "لعبة.خامسي.وقة.حظ.jpg", payouts: { 3: 2, 4: 5, 5: 10 } },
  { img: "لعبة.خامسي.توت.jpg", payouts: { 3: 3, 4: 7, 5: 14 } },
  { img: "لعبة.خامسي.سبعة.jpg", payouts: { 3: 4, 4: 9, 5: 18 } },
  { img: "لعبة.خامسي.خوخ.jpg", payouts: { 3: 5, 4: 11, 5: 22 } },
  { img: "لعبة.خامسي.توت.بري.jpg", payouts: { 3: 7, 4: 15, 5: 30 } }
];
...
function generateFinalResult() {
  ...
  for (let col = 0; col < COLS; col++) {
    ...
    for (let row = 0; row < ROWS; row++) {
      ...
      if (
        symbol.img === "لعبة.خامسي.وقة.حظ.jpg" &&
        (col === 0 || col === COLS - 1)
      ) {
        // نعيد الاختيار مرة ثانية بدون Wild
        do {
          symbol = getRandomSymbol();
        } while (symbol.img === "لعبة.خامسي.وقة.حظ.jpg");
      }
...
function highlightWild() {
  ...
  reels.forEach(reel => {
    ...
    if (hasWild) {
      ...
      imgs.forEach(img => {
        img.src = "../images/لعبة.خامسي.وقة.حظ.jpg";
...
let hasWild = symbols.some(s => s.img === "لعبة.خامسي.وقة.حظ.jpg");
...
if (hasWild) {
  isWildColumn = true;
    ...
    for (let i = 1; i < symbols.length; i++) {
      ...
      if (symbols[i].img === "لعبة.خامسي.وقة.حظ.jpg") {
        match++;
        continue;
      }
...
if (match >= 3) {
  ...
  if (symbols.some(s => s.img === "لعبة.خامسي.وقة.حظ.jpg")) {
    winningImg = "لعبة.خامسي.وقة.حظ.jpg"; // يعتبر Wild هو الفائز
  } else {
    winningImg = base.img;
  }
  ...
  if (winningImg) {
    ...
    highlightWild();
      if (winningImg === "لعبة.خامسي.وقة.حظ.jpg") {
    wildSound.currentTime = 0;
    wildSound.play(); 
   }
    }
